...
This project is built on a modern React Native and Expo stack:

- **Framework:** React Native with Expo SDK 54 and Expo Router for file-based navigation.
- **Language:** TypeScript for app logic, UI, and strong typing across the codebase.
- **State Management:** Redux Toolkit, React Redux, and `redux-persist` for global state and session persistence.
- **Data Fetching & Caching:** RTK Query with a custom re-authentication flow and `async-mutex` for token refresh safety.
- **UI & Navigation:** React Navigation, native gesture handling, bottom sheets, safe-area support, and animated transitions.
- **Charts & Visualization:** `react-native-wagmi-charts` and `react-native-svg` for portfolio and market visuals.
- **Security & Native Features:** Expo Secure Store, Clipboard, Haptics, Notifications, Document Picker, and Web Browser integrations.
- **Validation & Forms:** Zod plus the TanStack form adapter for schema-driven forms and validation.
- **Tooling:** ESLint, TypeScript, Expo Dev Client, and React Native SVG Transformer for a smooth development workflow.
...
_Once Metro starts, you can scan the QR code displayed in the terminal using your camera app (iOS) or the Expo Go app (Android)._
